...
public:
  void Book(int64_t time, const string& hotel_name,
    ...
    current_time_ = time;
    hotels_[hotel_name].Book({time, client_id, room_count}); // LlogQ + logC
  }
  int ComputeClientCount(const string& hotel_name) { // LlogQ + QlogC
    return hotels_[hotel_name].ComputeClientCount(current_time_);
  }
  int ComputeRoomCount(const string& hotel_name) {  // LlogQ + QlogC
...
private:
  ...
  class HotelInfo {
  public:
    void Book(const Booking& booking) { //logC
      last_bookings_.push(booking);
      room_count_ += booking.room_count;
      ++client_booking_counts_[booking.client_id]; // logC
    }
    int ComputeClientCount(int64_t current_time) { // QlogC
      RemoveOldBookings(current_time);
      return client_booking_counts_.size();
    }
    int ComputeRoomCount(int64_t current_time) { // QlogC
      RemoveOldBookings(current_time);
      return room_count_;
    }
  private:
...
int main() {
  ...
  for (int query_id = 0; query_id < query_count; ++query_id) {
    ...
    if (query_type == "BOOK") {		// LlogQ + QlogC
      int64_t time;				// L + QlogC
      cin >> time;
      string hotel_name;
      cin >> hotel_name;
