...
void TestSpeedMax() {
    ...
    LOG_DURATION("Speed 50k");
    TotalDuration make_ss_queries("making queries total");
    TotalDuration make_ss_docs("making docs total");
    TotalDuration procede_queries("queries total");
    TotalDuration index_docs("indexing total");

    SearchServer sv;

    for (int i = 0; i < 50; i++) {
        cout << i << endl;
        istringstream doc_input("");
        {
            ADD_DURATION(make_ss_docs);
            doc_input = MakeStream(GenerateDocs(words));
        }
        {
            ADD_DURATION(index_docs);
            sv.AppendDocumentBase(doc_input);
        }
    }
    for (int i = 0; i < 50; i++){
        //if (i % 10000 == 0) {
        ...
        istringstream query_input("");
        //vector<ostringstream> oss(50);
        ostringstream results_output;
        {
            ADD_DURATION(make_ss_queries);
            query_input = MakeStream(GenerateQueries(words));
        }
        {
            ADD_DURATION(procede_queries);
            sv.AddQueriesStream(query_input, results_output);
        }
    }
}
